...
window.addEventListener('DOMContentLoaded',async function(){chat=$('chat');document.addEventListener('click',function(event){var target=event.target.closest('[data-quick]');if(target)quick(target.dataset.quick)});$('sendEmployeeCommand').addEventListener('click',onSend);try{await login();$('uName').textContent=emp.user.displayName;aiBubble('<h5>你好，'+esc(emp.user.displayName)+'</h5>我是连接真实 WFM 数据和劳动力规则引擎的员工伙伴。查班会直接返回；请假、换班、补卡、加班、打卡和排班偏好会在你确认后写入系统。')}catch(error){aiBubble('<h5>登录失败</h5>'+esc(error.message))}$('input').addEventListener('keydown',function(event){if(event.key==='Enter'){event.preventDefault();onSend()}})});
