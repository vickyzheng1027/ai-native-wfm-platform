...
export async function parseDemandWithAgent(text,metadata){return withRetry(`主管业务输入：${text}\n可用元数据：${JSON.stringify(metadata)}`,'你是WFM业务需求理解Agent。只提取用户明确表达的信息，禁止创造门店、岗位或数字。输出JSON：items数组，每项必须包含storeCode、date（YYYY-MM-DD）、startTime（HH:mm）、endTime（HH:mm）、role、requiredCount、forecastTraffic、forecastSales；unresolved数组；summary。门店和岗位只能来自元数据。');}
