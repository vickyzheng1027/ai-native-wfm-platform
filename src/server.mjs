...
import { parseRulesWithAgent,parseDemandWithAgent,isAiConfigured,MODEL } from './openai-agent.mjs';
...
async function api(req,res,path,url){let p,b;
 ...
 if(req.method==='POST'&&path==='/api/demands/parse'){b=await body(req);const text=String(b.text||'').trim();if(!text)throw wfm.err('请输入业务需求',400);let items,summary='',unresolved=[],fallbackReason,modelSource='gaia_agent';try{const metadata=wfm.metadata(db),result=await parseDemandWithAgent(text,metadata);items=(result.items||[]).map(x=>{const store=metadata.stores.find(s=>s.code===x.storeCode);if(!store||!metadata.roles.includes(x.role)||!/^20\d{2}-\d{2}-\d{2}$/.test(x.date)||!/^(?:[01]\d|2[0-3]):[0-5]\d$/.test(x.startTime)||!/^(?:[01]\d|2[0-3]):[0-5]\d$/.test(x.endTime)||Number(x.requiredCount)<1)throw wfm.err('模型返回的业务需求未通过元数据校验',422);return {storeId:store.id,date:x.date,startTime:x.startTime,endTime:x.endTime,role:x.role,requiredCount:Number(x.requiredCount),forecastTraffic:Number(x.forecastTraffic||0),forecastSales:Number(x.forecastSales||0),source:'agent'};});if(!items.length)throw wfm.err('模型未识别出完整需求',422);summary=result.summary;unresolved=result.unresolved||[];}catch(e){items=schedule.parseDemandText(text,db);modelSource='deterministic_fallback';fallbackReason=e.message;summary='模型调用失败，已用本地规则提取，请人工核对后确认。';}return json(res,200,{ok:true,data:{items,modelSource,model:modelSource==='gaia_agent'?MODEL:null,summary,unresolved,fallbackReason}});}
