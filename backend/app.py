...
class Handler(BaseHTTPRequestHandler):
    ...
    def handle_api(self,method,path,query):
        ...
        if method=="GET" and path=="/api/metadata":return 200,{"ok":True,"data":{"stores":[dict(x) for x in DB.execute("SELECT * FROM stores ORDER BY code")],"ai":{"enabled":AIClient(DB).enabled,"model":AIClient(DB).model,"mode":"live_llm_rag" if AIClient(DB).enabled else "retrieval_only"},"roles":["admin","manager","hr","auditor","employee"]}}
